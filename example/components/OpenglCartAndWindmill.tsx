...
// コンポーネントを定義
export const OpenglCartAndWindmill: FunctionComponent<Props> = (props) => {
  // ここでクラス名を取得
  ...
  function drawWheel() {
    ...
    for (let i = 0; i < 15; i++) {
      glVertex2f(0, 0);
      glVertex2d(
        Math.cos((i * 2 * Math.PI) / 15),
        Math.sin((i * 2 * Math.PI) / 15)
      );
    }
    glEnd();
  }
  ...
  function init() {
    ...
    try {
      glsimUse('glcanvas2'); // OpenGL will draw to the canvas with id="glcanvas".
    } catch (e) {
  ...
  return (
    <div className={classes.root}>
      ...
      <div id="canvas-holder">
        <canvas id="glcanvas2" width="700" height="500"></canvas>
      </div>
    </div>
  );
};
